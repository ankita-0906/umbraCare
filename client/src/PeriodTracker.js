...
const PeriodTracker = () => {
  ...
  const fetchPeriodEntries = async () => {
    try {
      ...
      const response = await axios.get('http://localhost:5000/api/period-tracker', {
        headers: {
          'x-auth-token': token,
        },
      });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editMode && currentEntryId) {
        await axios.put(`http://localhost:5000/api/period-tracker/${currentEntryId}`, periodData, {
          headers: {
            'x-auth-token': token,
        ...
        await axios.post('http://localhost:5000/api/period-tracker', periodData, {
          headers: {
            'x-auth-token': token,
            'Content-Type': 'application/json'
          }
